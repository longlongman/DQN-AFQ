...
NS_LOG_COMPONENT_DEFINE("TcpFairnessAndConvergence");
...
void throughput(Ptr<FlowMonitor> monitor,FlowMonitorHelper* flowmon){
    ...
    for(std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i){
        ...
        if((rbit = myRxBytes.find(i->first)) == myRxBytes.end()){
            ...
            std::cout << "  Time:       "<<i->second.timeLastRxPacket.GetSeconds()<<" Seconds\n";
            ...
            std::cout << "  Duration:   "<<i->second.timeLastRxPacket.GetSeconds() - i->second.timeFirstTxPacket.GetSeconds()<<" Seconds\n";
            std::cout << "  Time:       "<<i->second.timeLastRxPacket.GetSeconds()<<" Seconds\n";
            std::cout << "  TxOffered:  " << (i->second.txBytes - myTxBytes[i->first]) * 8.0 / (i->second.timeLastTxPacket.GetSeconds()- myTimeLastTxPacket[i->first] + 0.000001) / 1000 / 1000  << " Mbps\n";
            std::cout << "  Throughput: " << (i->second.rxBytes - myRxBytes[i->first]) * 8.0 / (i->second.timeLastRxPacket.GetSeconds()- myTimeLastRxPacket[i->first] + 0.000001) / 1000 / 1000  << " Mbps\n";
