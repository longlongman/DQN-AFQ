...
class QueueingLogicPriRL {
 ...
 public:
  ...
  int push_front(size_t queue_id, size_t priority, const T &item) {
    ...
    auto &q_info = queues_info.at(queue_id);
    auto &w_info = workers_info.at(worker_id);
    priority = (w_info.start_priority + priority) % nb_priorities;
    auto &q_info_pri = q_info.at(priority);
    LockType lock(w_info.q_mutex);
    if (q_info_pri.size >= q_info_pri.capacity) return 0;
    // size_t temp_capacity = q_info_pri.capacity - 1;
    // set_capacity(queue_id,temp_capacity);
    for(auto &temp_q_info_pri : q_info){
      temp_q_info_pri.capacity--;
    }
  ...
  void pop_back(size_t worker_id,size_t *queue_id,size_t *priority,T *pItem){
    ...
    while(true){
      if(w_info.size == 0){
        ...
        for(size_t pri = 0;pri < nb_priorities;pri++){
          ...
          // if(w_info.deficits[index] > 0){
          //   w_info.start_priority = index;
          //   if(q.top().send <= now){
          //     // w_info.deficits[index]--;
          //     queue = &q;
          //     break;
          //   }
          //   next = q.top().send;
    ...
    *pItem = std::move(const_cast<QE &>(queue -> top()).e);
    queue -> pop();
    if(queue -> size() == 0){
      w_info.start_priority = (w_info.start_priority + 1) % nb_priorities;
    }
    auto &q_info = queues_info.at(*queue_id);
    auto &q_info_pri = q_info.at(*priority);
    // size_t temp_capacity = q_info_pri.capacity + 1;
  ...
  // size_t get_capacity(size_t queue_id,size_t priority)const{
  //   size_t worker_id = map_to_worker(queue_id);
  //   auto &q_info = queues_info.at(queue_id);
  //   auto &q_info_pri = q_info.at(priority);
  //   auto &w_info = workers_info.at(worker_id);
  //   LockType lock(w_info.q_mutex);
  //   return q_info_pri.capacity;
  // }
 ...
 private:
  ...
  struct WorkerInfo {
    mutable std::mutex q_mutex{};
    mutable std::condition_variable q_not_empty{};
    mutable std::condition_variable q_rate_limit{};
    size_t size{0};
    size_t start_priority{0};
...
// logic queue with drr scheduling algorithm
template <typename T,typename FMap>
class QueueingLogicDrrRL{
  using MutexType = std::mutex;
  using LockType = std::unique_lock<MutexType>;

  public:
    QueueingLogicDrrRL(size_t nb_queues,size_t nb_workers,size_t capacity,FMap map_to_worker,size_t nb_priorities = 2)
                      : nb_queues(nb_queues),nb_workers(nb_workers),workers_info(nb_workers),map_to_worker(std::move(map_to_worker)),
                      nb_priorities(nb_priorities){
        auto now = clock::now();
        for(size_t i = 0;i < nb_queues;i++){
          QueueInfoPri v = {0,capacity,0,ticks::zero(),now};
  ...
  QueueingLogicDrrRL(const QueueingLogicDrrRL &) = delete;
  QueueingLogicDrrRL &operator = (const QueueingLogicDrrRL &) = delete;

  QueueingLogicDrrRL(QueueingLogicDrrRL &&) = delete;
  QueueingLogicDrrRL &&operator = (QueueingLogicDrrRL &&) = delete;
  ...
  private:
    ...
    struct QE{
      QE(T e,size_t queue_id,const clock::time_point &sent) : e(std::move(e)),queue_id(queue_id),send(send){}
    ...
    struct QueueInfo : public std::vector<QueueInfoPri>{
      QueueInfo(size_t nb_priorities,const QueueInfoPri &v) : std::vector<QueueInfoPri>(nb_priorities,v){}
      size_t size{0};
    };
    ...
    struct WorkerInfo{
      ...
      size_t size{0};
      size_t start_priority{0};
      // std::array<uint64_t,32> deficits{{100000,100000,100000,100000,100000,100000,100000,100000,
    ...
    clock::time_point get_next_tp(const QueueInfoPri &q_info_pri){
      return std::max(clock::now(),q_info_pri.last_sent + q_info_pri.pkt_delay_ticks);
    }
